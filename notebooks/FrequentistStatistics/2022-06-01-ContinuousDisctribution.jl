...
# ╔═╡ 743c102e-e150-11ec-2bcb-95d67cc2a33c
using Distributions: Uniform, Exponential, Geometric

# ╔═╡ d7d5a45a-51cb-48fd-8a0c-1dab2da86466
using Distributions: pdf

# ╔═╡ 423c1734-d1b4-4448-b0f1-750f1f5cf8df
using Statistics: mean

# ╔═╡ 281e1fe4-da1d-4d2d-826e-012e59967ee0
using StatsBase: counts

# ╔═╡ 763cd0dd-a2c4-4afd-82a8-16a8014aba71
using GLMakie

# ╔═╡ bd8da8cb-5931-4d35-b50e-28d032c40ce0
using Distributions: Gamma

# ╔═╡ 01b2169e-7e87-43e9-9bba-fd1b0bc95f91
using Distributions: shape, scale

# ╔═╡ f5cb8c3d-bd01-40c8-8445-9107e62ba263
md"""
### 连续分布
"""

# ╔═╡ a051c183-3522-4096-8d3e-64d123ba9850
# @doc Uniform

# ╔═╡ 2c163a95-58e6-4031-a16a-e1f524d73baa
# @doc rand

# ╔═╡ 13cfb2e7-da8b-47a1-a73f-1896b5ae43a1
let
	cUnif = Uniform(0, 2π) # \pi + tab
	xGrid, N = 0:0.1:2π, 10^6
	fig = Figure(resolution = (1000, 500))

	ax = Axis(fig[1, 1], 
		xlabel = "范围", ylabel = "密度",
		xticks = (0:π/2:2π, ["0", "pi/2", "pi", "3pi/2", "2pi"])
	)

	hist!(ax, rand(N) * 2π, bins = xGrid, normalization = :pdf, label = "MC 估计")
	lines!(ax, xGrid, pdf.(cUnif, xGrid), color = :red, linewidth = 5, label = "概率密度函数")

	Legend(fig[1, 2], ax, "类型")

	fig
end

# ╔═╡ bc25ef59-0105-44d4-a0c0-a5a9bd52b2bf
md"""
## 指数分布
"""

# ╔═╡ 97584421-c155-4794-b1ed-bbcb33624e96
md"""
```math
...
# ╔═╡ 7adc5591-b24d-4e6b-bcb1-cf77186ea614
let
	lambda, N = 1, 10^2
	xGrid = 0:6
	...
	floorData = counts(Int.(floor.(rand(expDist, N))), xGrid)  / N
	
	geomDist = Geometric(1 - exp( - lambda))

	stem(xGrid, floorData, color = :blue, stemcolor = :blue, trunkwidth = 0)
	stem!(xGrid, pdf.(geomDist, xGrid), color = :red, stemcolor = :red, trunkwidth = 0)

	current_figure()
end

# ╔═╡ 80202e45-060c-4ca0-b5a1-d11c66fcd6da
md"""
## 伽马分布
"""

# ╔═╡ 1f937dad-d5a5-4250-b782-4b1268ecff45
# @doc Gamma

# ╔═╡ 61fae715-a221-4bae-b8fa-7066805786d2
begin
	lambda, N = 1/3, 10^6
	bulbs = [1, 10, 50]
	xGrid = 0:0.1:10
...
# ╔═╡ 2f6976e3-2483-4bd7-9988-1180a60dd560
function normalizedData(d::Gamma)
	sh = Int(shape(d))
	data = [sum(-(1 / (sh * lambda)) * log.(rand(sh))) for _ in 1:N]
end

# ╔═╡ 846ae699-5965-421d-91dc-8acf30fe24bc
let
fig = Figure()
ax = Axis(fig[1, 1], limits = (0, maximum(xGrid), 0, 1))

	for n in bulbs
		dist = Gamma(n, 1 / (n * lambda))
		shp = string(Int(shape(dist)))
		scl = string(round(scale(dist), digits = 2))
		hist!(ax, normalizedData(dist), bins = 50, normalization = :pdf,
			label = "形状 = $(shp); 比率 = $(scl)"
		)
		lines!(ax, xGrid, pdf.(dist, xGrid), linewidth = 5, color = :black)
	end
